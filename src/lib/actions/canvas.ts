...
export const gridLines: Action<
	HTMLCanvasElement,
	Omit<Parameters<typeof drawLines>[0], 'canvas'> & { visibility?: boolean }
> = (canvas, params) => {
	const ctx = canvas.getContext('2d');
	if (!ctx) {
		throw new Error('Canvas is not a 2d canvas.');
	...
	if (params.visibility ?? true) drawLines({ ...params, canvas });

	return {
		destroy() {},
		update(params) {
			ctx.clearRect(0, 0, canvas.width, canvas.height);
			if (params.visibility ?? true) drawLines({ canvas, ...params });
		}
	};
};
