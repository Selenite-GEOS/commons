...
pub fn get_comment(node: &Node<'_, '_>) -> Option<String> {
    for sib in node.prev_siblings().skip(1) {
        if !sib.is_text() && !sib.is_comment() {
            break;
        }
        if sib.is_comment() {
            return sib.text().map(|s| s.to_string());
        }
    }
    None
}
...
pub fn get_documentation(node: &Node<'_, '_>) -> Option<String> {
    find_child(node, "annotation")
        ...
        .and_then(|node| node.text().map(|s| s.to_string())).or_else(|| get_comment(node))
}
